...
keep<-array(FALSE,nrow(mat.eval)); # Arreglo de falsos
head(keep)
...
### Cluster Dendogram
```{r}
# Puntos en dimensiones
# Sample → tratamientos/condiciones/experimentos en genes
# Neuron → genes en tratamientos/condiciones/experimentos
...
ggplot(filtered_data, aes(x = Tag, y = logFC, fill = Regulation)) +
  ...
  theme(
    ...
    legend.background = element_rect(fill = "transparent")              # Fondo de la leyenda transparente
  ) +
...
```{r}
# Crear listas de genes por zona y día para el día D0
DGE_Venn_data <- list(
  Abscisión = DGE_lists[['Abscisión_D0']],
  Distal = DGE_lists[['Distal_D0']],
  Proximal = DGE_lists[['Proximal_D0']]
)

# Crear el diagrama de Venn usando ggVennDiagram
ggVennDiagram(DGE_Venn_data, label = "count") + 
...
```{r}
# Diagrama de Venn para todas las zonas en D0 y D2
ggVennDiagram(DEG_0, label = "count") +
  scale_fill_gradient(low = "lightblue", high = "blue") +
  ggtitle("DGE Venn Diagram - Zonas y Días") +
  theme_minimal()

ggVennDiagram(DEG_2, label = "count") +
  scale_fill_gradient(low = "lightblue", high = "blue") +
  ggtitle("DGE Venn Diagram - Zonas y Días") +
  theme_minimal()
